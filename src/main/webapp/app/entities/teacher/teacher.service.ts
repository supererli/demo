...
export class TeacherService {
    ...
    protected convertDateFromClient(teacher: ITeacher): ITeacher {
        const copy: ITeacher = Object.assign({}, teacher, {
            teacherTime: teacher.teacherTime != null && teacher.teacherTime.isValid() ? teacher.teacherTime.toJSON() : null
        });
        return copy;
    }
    ...
    protected convertDateFromServer(res: EntityResponseType): EntityResponseType {
        if (res.body) {
            res.body.teacherTime = res.body.teacherTime != null ? moment(res.body.teacherTime) : null;
        }
        return res;
    }
    ...
    protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
        if (res.body) {
            res.body.forEach((teacher: ITeacher) => {
                teacher.teacherTime = teacher.teacherTime != null ? moment(teacher.teacherTime) : null;
            });
        }
        return res;
    }
